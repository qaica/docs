...
At the completion of Milestone 1, any LoRaWAN gateways running [gateway-rs](https://github.com/helium/gateway-rs) can route packets to and from the Helium Network **but not earn HNT.** This means a DIY gateway can send and receive data on the Network. These LoRaWAN Gateways running Gateway-rs are known as Data Only Hotspots.
